...
/// All current Gemini 2.x / 3.x models ship with a 1M context window, so we
/// return the same constant regardless of the concrete model string. The
/// `model` argument is kept so this can differentiate later models without
/// a signature change.
