                                        ...
                                        msg.get("content").and_then(|c| c.as_array())
                                    {
                                        ...
                                        if !texts.is_empty() {
                                            ...
                                            result.first_assistant_text =
                                                clean_chat_text(&normalized, 200);
                                        }
                                    }
                                }
...
fn extract_prompt_text(message: &Value) -> String {
    ...
    let terminal_safe = super::sanitize_terminal_text(result.trim());
    let redacted = super::redact_secrets(&terminal_safe);
    if redacted.is_empty() {
        return String::new();
    }
    // Skip prompts generated by abtop's own summary generation (claude --print)
    if redacted.contains("You are a conversation title generator") {
        return String::new();
    }
    truncate(&redacted, 50)
}
...
"#,
    ...
    #[test]
    fn test_parse_transcript_initial_prompt_redacts_secrets() {
        let mut file = tempfile::NamedTempFile::new().unwrap();
        write_lines(
            &mut file,
            &[
                r#"{"type":"user","timestamp":"2026-03-28T15:00:00Z","message":{"role":"user","content":"deploy with sk-proj-secret and \u202E unsafe marker"}}"#,
            ],
        );
        let result = parse_transcript(file.path(), 0);
        assert_eq!(
            result.initial_prompt,
            "deploy with [REDACTED] and  unsafe marker"
        );
    }
