// Centralized English UI strings.
...
static LOCALE_EN: LazyLock<std::collections::HashMap<&str, &str>> = LazyLock::new(|| {
    ...
    m.insert("detail.evidence", "EVIDENCE");
    m.insert("detail.matching", "matching");
    m.insert("detail.since", "since");
    m.insert("detail.observed", "observed");
    m.insert("detail.generation", "generation");
...
mod tests {
    ...
    #[test]
    fn lifecycle_status_labels_are_distinct() {
        let keys = [
            "sess.think",
            "sess.exec",
            "sess.wait",
